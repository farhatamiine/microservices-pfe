	...
	public void validateTransaction(ConsumerRecord<Long,String> consumerRecord) throws JsonMappingException, JsonProcessingException { 
		...
		if(lastVerificationMessage != null) { 
			...
			*/ 
			...
			AdjustmentEvent verificationAdjustmentEvent = objectMapper.convertValue(verificationMessage, AdjustmentEvent.class);
			...
			if(verificationMessage.getTotalAmount().compareTo(sumDrops) == 1) {
				...
				if(diffrentBetween.compareTo(new BigDecimal(maxValue)) == 1) {
					...
					caseMessage = "verification messasge is greater than the sum of drops and the different betwen them is also greater than max vendor value : " + diffrentBetween + " > " + maxValue + ". we are talking about a surplus in that case the merchant has been short credited.";
					...
					caseMessage = "verification messasge is greater than the sum of drops and the different betwen them is between max and min vendor value : "+ minValue + " < " + diffrentBetween + " < " + maxValue + ". we are talking about a surplus in that case the merchant has been short credited.";
					...
					caseMessage = "verification messasge is greater than the sum of drops and the different betwen them is less or equal min vendor value : "+ minValue + ". we are talking about a surplus in that case the merchant has been short credited."; 
				}
				...
				if(diffrentBetween.compareTo(new BigDecimal(maxValue)) == 1) {
					...
					String accountNumber = merchantDeviceProxy.getMerchantAccount(verificationMessage.getMerchantNumber(), DEBITED_TYPE);
					verificationAdjustmentEvent.setAccountNumber(accountNumber);

					caseMessage = "verification messasge is less than the sum of drops and the different betwen them is greater than max vendor value : " + diffrentBetween + " > " + maxValue + ". we are talking about a shortage, in that case the merchant has been over credited.";
					
				} else if(diffrentBetween.compareTo(new BigDecimal(maxValue)) == -1 && diffrentBetween.compareTo(new BigDecimal(minValue)) == 1) {
					/*  min value < X < max value.
						Credit customer DDA:
						- Create a case with the debit transaction as follow.
						- Create a debit adjustment transaction for GL purposes.
						- User must not process the transaction used.
					 */
					// Create Adjustment.
					verificationAdjustmentEvent.setTransferAmount(verificationMessage.getTotalAmount().subtract(sumDrops));
					verificationAdjustmentEvent.setTransferSign(TransferSign.D);
					...
					caseMessage = "verification messasge is less than the sum of drops and the different betwen them is between max and min vendor value : "+ minValue + " < " + diffrentBetween + " < " + maxValue + ". we are talking about a shortage, in that case the merchant has been over credited.";
					...
					/*  X <= min vendor value.
					    ...
					    - Retrieve the account of the vendor.
					    - Create a case with the debit transaction as follow.
					    - Create a debit adjustment transaction to the vendor DDA account and process it automatically.
					 */
					...
					caseMessage = "verification messasge is less than the sum of drops and the different betwen them is less or equal min vendor value : "+ minValue + ". we are talking about a shortage, in that case the merchant has been over credited."; 
				}
